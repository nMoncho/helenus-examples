...
object HotelsTestData {
  ...
  def insertTestData()(implicit session: CassandraSession, ec: ExecutionContext): Future[Unit] = {
    ...
    for {
      ...
      insertByPOI <- insertHotelByPOI
      insertPoi <- insertPoiByHotel
      insertH <- insertHotel
      insertAR <- insertAvailableRooms
      insertAm <- insertAmenities
    } yield {
      for {
        hotel <- Hotels.all
        poiName <- hotel.pois
        ...
        insertByPOI.execute(poiName, hotel.id, hotel.name, hotel.phone, hotel.address)(cqlSession)
        insertPoi.execute(poiName, hotel.id, poi.description)(cqlSession)
      }

      Hotels.all.foreach(h => insertH.execute(h.id, h.name, h.phone, h.address, h.pois)(cqlSession))
      ...
      } insertAm.execute(hotel.id, room.toShort, amenity.name, amenity.description)(cqlSession)
    }
